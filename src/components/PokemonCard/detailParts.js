...
import { getPokemonSpriteUrl } from '../../services/pokemon';
...
import { Id, ImageSection, Name, Types } from './primitives';
...
function MatchupBlock({ title, tip, types, loaded, tipOpen, onToggleTip }) {
  const names = (types ?? []).map(typeName).filter(Boolean);
...
function EvolutionTree({ stages, total, currentName, onPress, shiny }) {
  if (!stages) {
...
export function Body({ children }) {
  const { closeTips } = usePokemonCard('PokemonCard.Body');
  return (
    <ScrollView
      style={styles.scroll}
...
export function OfflineBanner() {
  const { fromCache } = usePokemonCard('PokemonCard.OfflineBanner');
  if (!fromCache) return null;
  return (
    <View style={styles.offlineBanner}>
      <MaterialCommunityIcons name="wifi-off" size={14} color={colors.textMuted} />
...
export function Hero({ children }) {
  usePokemonCard('PokemonCard.Hero');
  return <View style={styles.hero}>{children}</View>;
}

export function Generation() {
  const { generation } = usePokemonCard('PokemonCard.Generation');
...
export function ShinyToggle() {
  const { shiny, onToggleShiny } = usePokemonCard('PokemonCard.ShinyToggle');
  if (!onToggleShiny) return null;
  return (
    <Pressable
...
export function Identity() {
  usePokemonCard('PokemonCard.Identity');
  return (
    <Text style={styles.nameLine}>
      <Id style={styles.heroId} />
      <Text style={styles.heroId}> </Text>
      <Name style={styles.heroName} numberOfLines={1} />
    </Text>
  );
}

export function Artwork({ size = 168 }) {
  usePokemonCard('PokemonCard.Artwork');
  return <ImageSection size={size} artwork />;
}

export function HeroTypes() {
  usePokemonCard('PokemonCard.HeroTypes');
  return <Types style={styles.heroTypes} />;
}

export function Weaknesses() {
  const { matchups, showWeaknessTip, toggleWeaknessTip } = usePokemonCard(
  ...
  return (
    <MatchupBlock
      ...
      onToggleTip={toggleWeaknessTip}
    />
  );
}

export function Resistances() {
  const { matchups, showResistanceTip, toggleResistanceTip } = usePokemonCard(
  ...
  return (
    <MatchupBlock
      ...
      onToggleTip={toggleResistanceTip}
    />
  );
}

export function Immunities() {
  const { matchups, showImmunityTip, toggleImmunityTip } = usePokemonCard(
    'PokemonCard.Immunities',
  );
  return (
    <MatchupBlock
      title="Immune to"
      tip="These types deal no damage. An attack of one of these types has no effect on this Pokémon."
      types={matchups?.immunities}
      loaded={Boolean(matchups)}
      tipOpen={showImmunityTip}
      onToggleTip={toggleImmunityTip}
    />
  );
}

export function ExtrasError() {
  const { extrasFailed, extrasError, onRetryExtras } = usePokemonCard(
    'PokemonCard.ExtrasError',
  );
  if (!extrasFailed || !extrasError) return null;
  return (
...
export function Stats() {
  const { pokemon } = usePokemonCard('PokemonCard.Stats');
...
export function Abilities() {
  ...
  return (
    <View style={styles.section}>
      ...
      <ExtraFallback loaded={abilities != null} skeleton={<AbilityBlockSkeleton />}>
...
export function Dimensions() {
  const { pokemon } = usePokemonCard('PokemonCard.Dimensions');
...
export function Forms() {
  const { pokemon, varieties, onOpenPokemon, openForms, shiny } =
    usePokemonCard('PokemonCard.Forms');
...
export function FormsModal() {
  const { pokemon, varieties, showFormsModal, closeForms, onOpenPokemon, shiny } =
    usePokemonCard('PokemonCard.FormsModal');
  ...
  return (
    ...
    >
      <View style={styles.modalRoot}>
        <Pressable style={styles.modalBackdrop} onPress={closeForms} />
...
export function TipDismiss() {
  const { anyTipOpen, closeTips } = usePokemonCard('PokemonCard.TipDismiss');
  if (!anyTipOpen) return null;
  return (
    <Pressable
      collapsable={false}
